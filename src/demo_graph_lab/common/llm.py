...
def chat(messages: list, run_dir: Path, tag: str, model: str | None = None,
    ...
    model = model or os.environ.get("DGL_VLM_MODEL", "anthropic/claude-opus-4.8")
    client = OpenAI(
    ...
    for attempt in range(retries + 1):
        try:
            ...
            total = artifacts.append_cost(run_dir, {
                "tag": tag, "model": model, "sec": round(time.time() - t0, 1),
                "prompt_tokens": u.get("prompt_tokens"),
                "completion_tokens": u.get("completion_tokens"),
                "cost": (u.get("cost") or 0.0),
            })
            if total > artifacts.cost_cap():
                raise CostCapExceeded(
                    f"cumulative cost ${total:.2f} > cap ${artifacts.cost_cap():.2f}")
            return content
