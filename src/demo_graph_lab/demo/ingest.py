"""Locate a task video and trace, then create a run with sampled frames."""
...
def find_video(task: str) -> Path:
    d = artifacts.data_root() / "data/filtered_1024_task_videos" / task
...
def find_trace(task: str) -> Path | None:
    ...
    base = artifacts.data_root() / "outputs/gemini35_eef_trace_v2_refined" / task
    if not base.exists():
        return None
...
def sample_frames(
    video: Path, out_dir: Path, n: int = 24, max_width: int = 640
...
def run(task: str, video: str | None = None, trace: str | None = None,
    ...
    run_dir = artifacts.new_run_dir(task)
