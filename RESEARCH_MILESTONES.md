...
**结论先行**：本文件是 v1（`RESEARCH_PROPOSAL.md`，2026-07-26）时代的 M1–M4 周排期，**执行策略整体作废**，由 `RESEARCH_PROPOSAL_V2.md`（2026-07-29）的 Phase 0/1/2 取代；**止损判据与验收阈值仍然有效**，本文件继续作为闸门规范源被引用。下按「有效 / 作废 / 歧义 / 建议 / 未核实」五节逐条列明。

- 盘上事实：本文件 mtime 停在 2026-07-26 16:02，共 164 行；全文 0 次出现 "Phase"（`grep -c Phase RESEARCH_MILESTONES.md` → 0），而 v2 的执行单位已是 Phase 0/1/2（`RESEARCH_PROPOSAL_V2.md:151`、`:203-207`）。
- `RESEARCH_PROPOSAL_V2.md:207` 写明「里程碑与止损沿 `RESEARCH_MILESTONES.md` 框架按 RSS 2027 倒排（**另文更新**）」——该「另文」至 2026-07-30 未产出。本块是这笔欠账的临时补丁，**不是**那份新排期。
...
| 项 | 本文件位置 | 状态 | 依据 |
|---|---|---|---|
| **H1 生死线**（组 3 对组 1/2 的 held-out gap） | 第 154 行闸门总表 | ✅ 判据不变 | v2 §2 原样保留 H1（`RESEARCH_PROPOSAL_V2.md:54`）；触发时点改读 v2 Phase 2 冻结协议主实验（`RESEARCH_PROPOSAL_V2.md:206`），不再是「M2.a」 |
| **消融 A 开关**（推导 Spec vs 手调常数：有差异→「推导」主张成立） | 第 104、152 行 | ✅ 开关逻辑不变 / ⚠️ 宿主改挂 | 原挂「伺服层」；v2 把伺服并入 H3' 的阶段内有界修正与「连续绑定」档位（`RESEARCH_PROPOSAL_V2.md:56`、`:133`），消融 A 应改挂该档位 |
| **消融 B 开关**（几何过滤无增量→保留 VLM 单级） | 第 97、153 行 | ✅ 开关逻辑不变 / ⚠️ 层数口径改 | v2 §4.2 是**三层**漏斗（硬可行性 / demo 几何谓词 / 下游约束反推，`RESEARCH_PROPOSAL_V2.md:107-118`）；本文件的「两级漏斗」按三层读 |
...
| **冻结协议**（进入 held-out 后 policy/模型/配置/runtime 全禁改，违反即数字作废） | 第 162 行 | ✅ | v2 效果层北极星沿用（`RESEARCH_PROPOSAL_V2.md:18`） |
| **产物纪律**（有效实验 24h 内更新 `PROGRESS.md`；`runs/` 不入 Git；push 前过 `public_release_check.py`） | 第 160、163 行 | ✅ | — |
| **M0 证据链**（B7 / slotgeom / B4-probe / B5.1） | 第 32 行 | ✅ 已升格为 v2 的独家证据节 | `RESEARCH_PROPOSAL_V2.md:38-42`，数字与本文件一致 |
...
2. **ICRA 2027 stretch**（第 20 行）❌。v2 定唯一投递目标 **RSS 2027**，ICLR 2027 亦明确排除（`RESEARCH_PROPOSAL_V2.md:5`；附录 A `:248`：v1「RSS/ICRA 2027」→ v2「RSS 2027（定）」）。
3. **H3 作为独立闸门**（7 处：第 41、47、51、86、99、105、149 行）❌。v2 §2 用 **H3'** 取代：两级 ReAct = 阶段间 gate（验收不过不放行）+ 阶段内有界修正（残差 = 约束 − 现状）；伺服（连续闭环）降为绑定档位中的「连续绑定」一档，**不再单独立闸门**（`RESEARCH_PROPOSAL_V2.md:56`、`:133`；附录 A `:248`）。**直接后果**：§M2.c 的入口条件「仅当 H3 开启」与第 105 行「H3 关闭时整节跳过」**双向悬空**；该节 `ServoSpec` TODO 需按绑定档位（静态 / 入口绑定 / 连续绑定 + 失败升档）重写；第 86 行「组 4 vs 5：闭环增量（视 H3）」改读为 H3' 的 gate/修正消融。
4. **「仅 1022 / 唯一工作树」工作边界**（第 30、161 行）❌。2026-07-29 起共同实验机 = 5090 `knowin-sim@192.168.77.113`，1022/学生区退为只读 upstream（`RESEARCH_PROPOSAL_V2.md:211-213`）。**须分开两件事**：作废的只是「1022 是唯一工作树」；同一行的「1024 NAS 只读、禁写入/部署」仍然有效（`PROGRESS.md` §硬边界更正），「GT 防火墙常开」「发控制必经用户批准」两条同样 ✅ 不变。
5. **配套文档指针**（第 4 行「配套文档：`RESEARCH_PROPOSAL.md`」）⚠️ 改读 `RESEARCH_PROPOSAL_V2.md`；v1 仅保留为方法主张与证据索引的历史底本（v2 附录 A `:250` 声明证据索引沿用 v1 附录 A，不重复维护）。
...
> ⚠️ **2026-07-30**：本行已作废。唯一投递目标 = **RSS 2027**，ICRA 2027 stretch 与 ICLR 2027 均取消（`RESEARCH_PROPOSAL_V2.md:5`、附录 A `:248`）；上方 M1–M4 / W1–W16 排期整体由 Phase 0/1/2 取代。见顶部 SUPERSEDED 声明 §S2-1、§S2-2。
...
> ⚠️ **2026-07-30**：入口条件「仅当 H3 开启」已**悬空**——v2 §2 用 **H3'**（两级 ReAct：阶段间 gate + 阶段内有界修正）取代 H3，伺服降为「连续绑定」档位，不再单独立闸门（`RESEARCH_PROPOSAL_V2.md:56`、`:133`）。本节的**消融 A 仍然有效**，但须改挂到 H3' 的阶段内有界修正；`ServoSpec` 相关 TODO 需按绑定档位重写。见顶部 SUPERSEDED 声明 §S2-3。
...
> ⚠️ **2026-07-30**：本表 **H3（伺服）** 行整行作废（→ H3'，见顶部 §S2-3）；其余五行判据**仍然有效**，是本文件继续被引用的主要价值。但「触发时点」列里的 M1.a / M1.c / M2.a 是 v1 标签，与 `harness/PHASE1_API_PLAN.md` 的 M1a/M1b/M1c 不是同一件事（§S3）；按 v2，H1 的触发时点改读 Phase 2 冻结协议主实验（`RESEARCH_PROPOSAL_V2.md:206`）。
...
2. 仅 1022；1024 NAS 只读；GT 防火墙常开；发控制必经用户批准
   > ⚠️ **2026-07-30**：「仅 1022」已作废——共同实验机改为 5090 `knowin-sim@192.168.77.113`，1022 / 学生区退为只读 upstream（`RESEARCH_PROPOSAL_V2.md:211-213`）。同行其余三条（1024 NAS 只读、GT 防火墙常开、发控制必经用户批准）**仍然有效**。见顶部 §S2-4。
