"""kwadapter._move 的运动规划接线单测(EP-1 交付物 1)。

判据(docs/DECISIONS.md D-09「放行 motion planning 代替 raw IK」):
...
from harness import kwadapter, robotapi
...
class MovePipe:
    ...
    get_qpos 同理**一发即到位**(qpos_move 后立刻回读为目标航点),否则 robotapi._wait_qpos
...
class StillEval:
    ...
    形状按 kwadapter._arm_qpos 的切片口径(两臂交错 + 后续自由度)给足。"""
...
def _rt(pipe):
    """真 KWRuntime,但 pipe/eval 换成替身;graph 为空(_move 不读图)。"""
    rt = kwadapter.KWRuntime.__new__(kwadapter.KWRuntime)
...
START_XQUAT = [0.10, -0.10, 1.10] + list(kwadapter.TDX0)
REACHED_XQUAT = TARGET_XYZ + list(kwadapter.TDX0)
...
def test_move_prefers_motion_planning_path():
    ...
    ok = rt._move(TARGET_XYZ, quat=kwadapter.TDX0)
    ...
    assert plan_kwargs["data"][3:7] == pytest.approx(list(kwadapter.TDX0))
...
def test_move_quat_none_uses_vertical_pose_as_goal():
    ...
    expected_q = kwadapter._topdown_like(START_XQUAT[3:7])
...
def test_servo_path_still_available_directly():
    """退化路径保留为独立方法(_move_servo),行为与接线前一致(只走 xquat_move)。"""
    pipe = MovePipe(_mp_result(), START_XQUAT, reach=REACHED_XQUAT)
    rt = _rt(pipe)

    rt._move_servo(TARGET_XYZ, quat=kwadapter.TDX0)
