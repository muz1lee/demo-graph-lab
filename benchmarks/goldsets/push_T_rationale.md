...
- t0000_00: red T-block sits right of the gray T-shaped target marker; initial orientation only mildly off (~-11° by pixel measurement).
...
- t0005_12–t0007_38 (s1): EE nudges/rotates the block with side contacts at table level (wrist overhead, contact horizontal; the contact normal makes this a side approach).
...
Constraints:
- `approach_direction(cone=side, t_block_red)` → **correct**. t0001/t0002: horizontal side-face contact drives the push; top-down contact cannot translate the block. The cone is judged by contact normal, not wrist pose. Core for non-prehensile pushing.
...
Acceptance:
- `axis_parallel(long_axes), holds=at_end` → **wrong**. Directly contradicted at s0 end: t0003/t0004 show the block grossly rotated vs the marker (~+120°). The coarse-push stage ends before orientation alignment is complete.
...
Constraints:
- `approach_direction(cone=side, t_block_red)` → **correct**. t0005_12–t0007_38: overhead wrist but side-face contacts at table level (clearest t0007_38). Required for planar fine pushes/rotations.
...
- `axis_parallel, holds=at_end` → **correct**. Marker hidden under the block ⇒ orientation matched (~+2°). T-block is not rotationally symmetric, so orientation is core.
...
1. **Scope discipline is the dominant error mode**: acceptance relations are copied into per-stage constraint lists without `holds`, producing during-stage claims the frames contradict (s0 center_align, s0-acceptance & s1-constraint axis_parallel). The `holds` field is only populated on acceptance items.
...
4. **Cone-by-contact-normal is applied correctly**: both stages report `side` despite the overhead wrist.
