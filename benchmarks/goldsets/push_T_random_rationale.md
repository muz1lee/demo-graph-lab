...
这个 demo 中，s0 只完成粗定位和部分旋转，交接时朝向残差约 90°，主要旋转由 s1 完成。因此粗容差交接只对**位置**成立，不对朝向成立。
...
| acceptance `axis_parallel(long_axis)` holds:at_end | **wrong** | 被自己的证据帧（81/100 = `t0004_00`）直接矛盾：s0 末块长轴（横梁）近水平、pad 长轴竖直，残差 ~90°。在 holds 范围内证据相反，因此判为 wrong。 |
| acceptance `center_align(center)` holds:at_end | correct | `t0004_00`：块脚印落在印记上，中心偏差约半个块宽；作为粗容差交接条件有帧支持且 load-bearing（s1 只做局部推）。 |

missing：无。s0 交接不含朝向条件（残差 ~90° 是演示事实），不补朝向类项。
...
- **数据质量问题**：`stage02/t0009_12.jpg`（stage end_sec=9.12s 的最后一个关键帧）是**另一段视频**（木地板 + 存钱罐场景）——源视频在 ~9s 处切换到下一段 clip，stage 2 的时间窗越过了剪辑点。当前 s2 无约束受其影响，故不产生 wrong/unsure，但关键帧采样和 stage 终点应加镜头切变检测，否则 cleanup 阶段可能抽出跨场景的幻觉约束。
- missing：无（顺序项已记在 s1）。
