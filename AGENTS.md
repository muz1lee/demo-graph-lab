...
本仓是公开、净化后的 source-of-truth。对外名是 **demo-graph-lab**（不是 ksm）。技能迭代、
感知 probe、抓取试验与模块化改造的**唯一工作树**是 1022 上的
`/mnt/data/wenqian/demo-graph-lab`（及其同步镜像）。

1024 NAS 基础仓 `/mnt/nas/knowin_sim/sim_workspace/` 不是本项目迭代场地：可只读借用其中的
