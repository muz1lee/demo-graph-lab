...
- `docs/STACK.md` 写技术栈选型定案（唯一选型权威源；改选型须先在 `docs/DECISIONS.md` 新增裁决再回填）。
- `docs/DECISIONS.md` 是唯一裁决台账，只追加新裁决或状态块。

过期内容直接删除或合并，不维护历史状态链。**例外**：`docs/DECISIONS.md` 与 `docs/archive/` 属裁决与理论存档，只追加或加状态块，不删正文（D-16 延续，见 D-25）。
