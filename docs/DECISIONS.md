...
| **状态** | **生效（②部分撤销：2026-08-03，D-24）**——**正文一字未删（D-16）**。撤销面**只有一处**：裁决栏②与理由栏②的**「仿真状态克隆预演」整个模块**（含 §2.1 的 fork Genesis 虚拟执行、$\max_{c'}F_{i+1}$ 的「精确计算」升格、以及以它为前提的「全场空位」新颖性主张与 WAP 预期增益推算）。PI 理由：**已经在仿真里跑了，再克隆一个预演太重，且有循环论证嫌疑**（在仿真里用仿真预演——原 v5 §5 已把这条列为风险并只给了「只产排序建议」的软止损，D-24 判定该止损不足以支撑把它当核心模块）。**下游可行性回归 `compat(c,c')` 几何兼容性谓词**（v4 §2.1 的采样近似，公式与消融开关本就未变），由 **T-BP** 承载；**T-SIM 从 §2C 占位表划掉**。**其余全部不变且仍生效**：①运行时 VLM 回归（三处入口不变）、③成本约束撤销、④消融阶梯与「有链 vs 无链」成对归因主线（阶梯第②级由「+预演」改为「+compat 前瞻」，级数与配对逻辑不变）、⑤玩耍只留接口、⑥八篇背景。D-01 的「已撤销」与 D-18 的「部分撤销」**不受影响**（那两条由 D-23① 触发，与被撤销的②无关）。 |
...
### D-24 技术栈冻结：GraspNet 升主源、点云链立即建设、仿真预演砍除、运行时 VLM = OpenRouter Opus

| | |
|---|---|
| **裁决** | **冻结技术栈**，新建 `docs/STACK.md` 作为**唯一选型权威源**。四项拍板：<br>① **抓取候选主源 = GraspNet**（代码在上游 1022 服务器 wht 工作区，**部署勘察另行进行**）；`qwen_dof_xquat` 由主源**降为备用/对照源**。<br>② **深度 / 点云链立即建设**：`get_depth`（bulk）→ `mask_to_points` → `compute_obb`（原 P1-05/06/07 提前，不再等 L4）。<br>③ **D-23② 的「仿真状态克隆预演」砍除**——下游可行性检查**降级回几何兼容性谓词 `compat(c,c')`**（= v4 §2.1 的采样近似方案），由 **T-BP** 承载；**T-SIM 工作流占位划掉**。<br>④ **运行时 VLM = OpenRouter Opus**（`anthropic/claude-opus-4.8`），出现在**三处且仅三处**：L4③ 选择深思 / L6② 验证证据 / L7 有界修正。<br>附立两条纪律：**冻结纪律**（本表为唯一权威源；执行任务书第一行引用本表；改选型须新增 DECISIONS 裁决，不许就地改表）+ **最小验证门纪律**（上机任务必须带 **touch-test 级**最小验证门，infra-first）。 |
| **日期** | 2026-08-03（PI 叫停执行后拍板） |
| **背景** | PI 叫停执行，判此前是**「盲搞」**，根因诊断为**选型未冻结**：同一个槽位（抓取候选源 / 感知链 / 前瞻机制 / 运行时模型）在 `PROPOSAL.md`、`ARCHITECTURE.md`、`TODO.md` 三份文档里各写各的，执行 agent 拿到任务书时要自己猜用哪个，猜错了也没人拦。**EP-2 是这个病的实证**（`harness/EP2_REPORT.md`）：三层执行 infra bug 叠加，全部不是方法问题——① `get_ee_extforce` 返回 numpy `str()` 形态解析失败恒为 `None`，**伪装成「这个仿真没有力反馈」**并长出一整套兜底逻辑；② `CLAW_TIP_DZ` 差 **6.2 cm 且符号反**，而这个错值**正是从坏掉的力读数里标出来的**；③ 「关节已收敛」被当到位证据，实际末端横向偏 **9.5 cm**。三条消耗了整轮实验预算，却与研究主张无关。至于②的砍除，PI 理由大意：**已经在仿真里跑了，再克隆一个预演太重，而且有循环论证嫌疑**。 |
| **理由** | ① **选型是执行的前置条件，不是执行的产物**——不冻结就等于每个执行 agent 各自重做一次选型决策，错误不可归因、返工不可预防；冻结成一页表，冲突面一眼可见。② **预演的成本/收益在我方场景里不成立**：v5 把它当「全场空位」是相对 WAP 的 learned world model 讲的，但我方**本来就在仿真里**——fork 一份仿真去预测仿真，收益是「把已经能算的东西再算一遍」，成本是每决策点一次完整 rollout；且论文面上「在仿真里用仿真预演」的循环论证质疑（v5 §5 已自列为风险）用「只产排序建议」挡不住。**`compat(c,c')` 才是脊椎里本来就有的东西**——v4 §2.1 的 $F_i$ 递归从未依赖预演，采样近似完全够用，砍掉预演**不动脊椎一行公式**。③ **GraspNet 升主源是纠正一个错配**：`qwen_dof_xquat` 是 VLM 派生的抓取决策（内含物体尺寸估计与姿态决策，`PRIMITIVE_API.md` 明确「不是原语」），拿它当**候选源**等于让同一族模型既产候选又被 VLM 深思排序，候选多样性与归因干净度双输；GraspNet 是几何方法，与我方漏斗的 L1 硬可行层同构。④ **最小验证门是 EP-2 教训的制度化**：坏读数比没有读数更危险，因为它会**自我繁殖出坏常数**；廉价的 touch-test 能在花掉整轮预算之前把这类问题挡住。 |
| **影响** | ① **新建 `docs/STACK.md`**（本裁决的落点，97 行）；此后「用哪个」只看它，`PROPOSAL.md`/`ARCHITECTURE.md`/`TODO.md` 与之冲突以它为准。② **D-23 状态改「生效（②部分撤销：D-24）」**，正文一字未删（D-16，见 D-23 状态栏）。③ **`PROPOSAL.md` 外科修订为 v5.1**：§0/§2 的 L4② 行改为「下游前瞻 = compat 几何谓词（T-BP）」；§2.1 整节改为**已砍除记录**（占位表作历史保留，标注砍除与理由）；消融阶梯第②级由「+预演」改为「+compat 前瞻」；§0.2「不声称发明预演」条收敛；八篇占位表 WAP 行改写（我方**既不用世界模型也不用仿真预演**，前瞻 = 约束链 + 几何谓词，轻量可审计）。④ **`TODO.md` 加 ⚡D-24 块**：新执行次序 = **①抬升打滑（EP-2 遗留）→ ②GraspNet 部署 → ③点云链 → ④VLM 深思接入**；**T-SIM 从 §2C 划掉**。⑤ **原 P1-11（GraspNet 对照臂，不在关键路径）定位作废**——GraspNet 进关键路径；`TODO.md` TL;DR-8「GraspNet 定位不变：P1 后段对照臂」一句随之失效。⑥ **`ARCHITECTURE.md` §A8 的「候选源双轨」主次翻转**（T1/T2 互换），该文件本轮不改，以 `STACK.md` 为准。⑦ **深度链提前**意味着 A7 `get_depth`（A 组唯一「真开发」，重）成为近期最大工程量单点。 |
| **状态** | **生效** |
| **证据** | 对话裁决（2026-08-03，PI 叫停执行后拍板），本文件为首个文档出处；落点 = `docs/STACK.md`；EP-2 三层 bug 与全部实测数字（力 0.22–0.40 N → 100.54 N、`CLAW_TIP_DZ` −0.010、9.5 cm → 8.6 mm、70.6 s → 40.0 s、夹爪 0=闭/100=开、抬升打滑未解）出处 `harness/EP2_REPORT.md`（✅实测）；`compat(c,c')` 公式出处 `archive/PROPOSAL_v4.md` §2.1；GraspNet 部署风险清单出处 `docs/ARCHITECTURE.md:84`。⚠️ **GraspNet 在 1022 的可部署性尚未验证**（勘察进行中），本条只冻结选型，不声称它已可用。 |
