> **迁移注(2026-08-04,D-25)**:本表随主线收敛迁入重构仓。文中代码路径按旧结构(`harness/*`)书写,新结构对应件见 `src/demo_graph_lab/` 与 `docs/API.md`;各槽位**选型定案与两条纪律(冻结纪律、最小验证门)继续有效**,改选型仍须先在 `docs/DECISIONS.md` 新增裁决。
