...
> | 编号 | 一句话 | 依赖 |
> |---|---|---|
> | ~~**T-SIM**~~ | ~~**预演引擎**：fork 当前 Genesis 状态，虚拟执行候选 + 后续链前缀，收集各阶段 gate 结果~~ ← **已砍除（D-24③）**，前瞻回归 `compat(c,c')` 谓词，由 **T-BP** 承载 | ~~效果里程碑（单集打通）+ 感知~~ |
> | **T-SEL** | **VLM 深思**：候选覆盖层渲染进场景图，对照 demo 关键帧与约束**排序**，必须引用支持该选择的约束；**不打分、不出连续量** | ~~T-SIM~~ → **L3 感知 + 真实执行 gate 结果**（D-24③） |
> | **T-COR** | **修正阶梯**四级：gate 重试 → 有界 VLM 修正 `{-1,0,+1}`×固定步长 → 回 L4 重选 → trace 引导代码修复（每 seed ≤3 次重放，debug/eval seed 严格分离） | ~~T-SIM~~、**T-SEL**（D-24③） |
...
> **⚡ D-24（2026-08-03，PI 叫停执行后拍板：技术栈冻结，覆盖上方 D-23 块与下文与之冲突的选型表述）**：
>
> **① 技术栈已冻结，唯一权威源 = `docs/STACK.md`。** 「这一槽用什么」只看它；本文、`PROPOSAL.md`、`ARCHITECTURE.md` 与它冲突，一律以它为准。**冻结纪律**：改选型不许就地改表，须先在 `DECISIONS.md` 新增裁决再回填。**最小验证门纪律**：任何上机任务书必须带 **touch-test 级**最小验证门（infra-first），且**第一行写明「选型依据 `docs/STACK.md` §<节>」**——没有这一行的任务书不许开工。
>
> **② 新执行次序（替代上方 D-23③ 的「修爪 → 单集 → 感知 → L4」）**：
>
> | 序 | 任务 | 状态 / 依据 |
> |---|---|---|
> | **①** | **抬升打滑解决**（EP-2 遗留） | 夹爪**已能夹住**（40/60 档实测），**抬升会滑脱、根因未查**（PI 叫停处）。这是唯一挡在「单集打通」前面的物理问题，出处 `harness/EP2_REPORT.md` §6 |
> | **②** | **GraspNet 部署**（升为候选**主源**，D-24①） | 代码在上游 **1022 服务器 wht 工作区**，**部署勘察进行中**。风险清单见 `ARCHITECTURE.md:84`（CUDA 扩展未编译 / `.venv` 无 pip / config 指 1021 沙箱 / smoke 输入 160×120 vs 相机 1280×720）——**2.0 人日是乐观值** |
> | **③** | **深度 / 点云链**（D-24②，即原 **P1-05 / P1-06 / P1-07**） | `get_depth`（bulk）→ `mask_to_points` → `compute_obb`。**编号与判据不变，只是提前**；`get_depth` 是 A 组唯一「真开发」（重） |
> | **④** | **VLM 深思接入**（T-SEL） | 运行时 VLM = **OpenRouter Opus**（`anthropic/claude-opus-4.8`，D-24④） |
>
> **③ T-SIM 从 §2C 的 L4 占位表划掉**（D-24③）：~~T-SIM 预演引擎~~ **砍除**——PI 理由：已在仿真中运行，再克隆一个预演太重，且有循环论证嫌疑。**下游可行性检查降级回 `compat(c,c')` 几何兼容性谓词**（v4 §2.1 的采样近似），**由 T-BP 承载**（T-BP 的 D-21 后置状态不变，本条只是把前瞻职责还给它）。§2C 其余四条（**T-SEL / T-COR / T-QC / T-ALIGN**）**保留**；T-SEL / T-COR 原写的「依赖 T-SIM」改读为**依赖真实执行的 gate 结果**（不是预演后果）。
>
> **④ 抓取候选源主次翻转**：**GraspNet = 主源**，`qwen_dof_xquat` **降为备用 / 对照源**。因此 **TL;DR-8「GraspNet 定位不变：P1 后段对照臂，不在关键路径」一句失效**——GraspNet **进关键路径**；原 **P1-11** 的「对照臂」定位随之作废（编号保留，定位以本块为准）。`ARCHITECTURE.md` §A8 的 T1/T2 主次同样翻转，该文件本轮未改，**以 `STACK.md` 为准**。
