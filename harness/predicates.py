"""[phase1 · predicates] 封闭词表 10 约束 → **三值**几何检验(P0-05,破口②)。

设计依据:docs/TODO.md §1.2/§2 P0-05、§3 D0 表 CC-3、docs/DECISIONS.md D-04/D-05。

━━ 为什么三值(违反即失败的设计律)━━
旧 `kwadapter.verify()` / `gates` 有五处 fail-open:检查不了(未覆盖谓词、异常、不可观测)
一律**静默返回 True**——「根本没检查」与「检查通过」在报表里成了同一个值(见 ARCHITECTURE §165 断点表)。
本模块把「检查不了」和「检查通过」在类型上分开:

    PASS    = 谓词成立(margin ≥ 0,数值是到边界的余量)
    FAIL    = 谓词违反(margin < 0,数值是越界的幅度)
    UNKNOWN = 检查不了(缺参照实体 / 缺输入 / 本 runtime 无法几何判)——**不改判定方向**

`UNKNOWN` 永远不等于 PASS,也不等于 FAIL。调用方(gate)按记账处理:计入 `constraint_ledger`
的 UNKNOWN 计数(CC-4 <20% 是 D0 闸门之一),但**不静默放行、不静默否决**。
`vacuous_pass` 语义(D-05:入口即为真的成立不带证据权重)由 gate 侧保留,本模块不涉及。

━━ 覆盖(CC-3:≥8/10 有可执行谓词;卡 carry/order 不算失败,region_grasp 必须可检查)━━
可几何检验(8):axis_vertical / axis_parallel / center_align / above / inside /
              region_grasp / approach_direction / clearance
本 runtime 不可检验(2,显式 UNCHECKABLE_IN_RUNTIME,计入报告,CC-3 允许):
  carry  —— 需夹爪-物体附着这个跨阶段状态量,几何快照读不出(CC-3 点名允许)。
  order  —— 需跨阶段执行序,单帧实体快照无此信息(CC-3 点名允许)。

━━ 纯函数纪律 ━━
本模块只吃 `entities` 快照(`{name: {"pos","quat","aabb"}}`,与 binding/regions 同一套),
不持有 runtime、不发网络、运行期零 LLM(D-01)。`region_grasp` 的归一化竖直带**复用**
`binding` 的规则(import,不重复实现);`approach_direction` 复用 `regions.cone_angle_deg`。
不含任何任务名/物体名/场景度量魔数——阈值是几何容差常量,不是场景坐标。
"""
...
# 本 runtime 结构上无法检验的约束(几何快照读不出的跨阶段状态)。CC-3 允许并计入报告。
...
def _quat_z_axis(q):
    """物体局部 +z 在世界系的方向(wxyz)。与 kwadapter._quat_to_z_axis 同式。"""
    w, x, y, z = q
...
def pred_axis_parallel(c, entities, **ctx):
    ...
    # axis_b 无实体或无 quat 时按世界竖直近似(与 kwadapter oracle 简化同规,显式记 detail)
...
def pred_region_grasp(c, entities, *, grasp_point=None, **ctx):
    ...
    (这是「输入缺失」,不是「不可检查」——本谓词有完整几何实现,CC-3 的可检查载体就是它)。
    rim/handle 走 binding/regions 的 UNCHECKABLE 集(几何特征检测 v1 不做)→ UNKNOWN。
    """
    a = _args(c)
...
def check(constraint, entities, **ctx) -> Predicate:
    ...
    entities   : {name: {"pos","quat","aabb"}} 实体快照(evaluator 侧特权量,见 D-04)。
    ctx        : 谓词专用输入,如 grasp_point=[x,y,z]、approach_dir=[x,y,z]。

    未覆盖名的处置(不 fail-open):
      • carry/order → UNKNOWN(reason="uncheckable_in_runtime",CC-3 允许并计入报告)。
