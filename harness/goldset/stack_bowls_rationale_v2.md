# stack_bowls gold v2 rationale (annotator: claude-bringup-v2, 2026-07-30)

Run: `harness_stack_bowls_20260730_004159`. Independent v0.2 annotation; v1 gold NOT consulted.
...
Standing rules applied (task brief + RUBRIC):
...
objects.json is the v0.2 fix for the v0.1 "one bowl, three names" failure, and it works here:
...
- `approach_direction top_down`: real in frames but grasp-side on a symmetric bowl → **incidental** per boss principle. An angled side pinch of the rim is an equally valid class member.
...
Cleanup extraction judged normally per v0.2 policy.
